...
namespace MyInventory.API.Tests.Steps
{
    ...
    {
        public static readonly string MyInventoryServerUrl = ConfigurationManager.AppSettings["MyInventoryServerUrl"];
        ...
        {
            ExecuteHttp(Method.POST, url, MyInventoryFeatureContext.Get().SubstitueKeys(bodyString));
        }

        [When(@"I PUT '(.*)' with the following:")]
        public void WhenIPUTWithTheFollowing(string url, string bodyString)
        {
            ExecuteHttp(Method.PUT, url, MyInventoryFeatureContext.Get().SubstitueKeys(bodyString));
        }

        [When(@"I DELETE '(.*)' with the following:")]
        public void WhenIDELETEWithTheFollowing(string url, string bodyString)
        {
            ExecuteHttp(Method.DELETE, url, MyInventoryFeatureContext.Get().SubstitueKeys(bodyString));
        }
        ...
        private static void ExecuteHttp(Method method, string url, string bodyString)
        {
            ...
            request.AddParameter("application/json", MyInventoryFeatureContext.Get().SubstitueKeys(bodyString), ParameterType.RequestBody);

            var response = new RestClient(MyInventoryServerUrl).Execute(request);
        ...
        {
            var resource = MyInventoryFeatureContext.Get().SubstitueKeys(url);
